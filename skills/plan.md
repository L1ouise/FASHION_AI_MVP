...
│
├── src/
│   ├── app.py                        # Streamlit main app (router + design system)
│   ├── auth.py                       # Login / Signup page
│   ├── utile.py                      # Shared utilities (Qdrant, CLIP, hashing, _get_secret)
│   ├── search.py                     # 3-tab semantic search (text / category / image)
│   ├── analytic.py                   # Plotly analytics page
│   ├── profile_ai.py                 # Full-page profile editor + signup form
│   ├── look_generator.py             # Morphology-aware outfit builder
│   ├── style_advisor.py              # Fashion advice engine (morphology + skin-tone)
│   ├── vton.py                       # Virtual Try-On (mannequin overlay)
...
│
├── docker-compose.yml
├── Dockerfile                        # Streamlit container (python:3.11-slim)
├── airflow/Dockerfile                # Airflow container (qdrant-client==1.7.3 pinned)
├── requirements.txt
...
# Airflow & Spark — install only in WSL2/Docker/Linux
# See requirements-airflow.txt or airflow/Dockerfile
```
...
## 13. Post-Implementation Update (v2)

> This section documents changes made after the initial plan was executed.

### Architecture refactored to modular pages

The monolithic `app.py` has been refactored into independent page modules:

| Module | Purpose |
|---|---|
| `app.py` | Thin router & design system injection (dark luxury CSS, sticky header) |
| `auth.py` | Login / Signup page (extracted from app.py) |
| `search.py` | 3-tab search (Text / Category / Image) |
| `look_generator.py` | Morphology-aware outfit builder |
| `style_advisor.py` | Fashion advice engine (morphology rules + skin-tone palettes) |
| `vton.py` | Virtual Try-On with mannequin overlay |
| `analytic.py` | Plotly dark-theme analytics (PCA scatter, radar chart) |
| `profile_ai.py` | Full-page profile editor + standalone signup form |
| `utile.py` | Shared helpers (Qdrant client, CLIP model, auth, `_get_secret()`) |

### Key dependency changes

- `qdrant-client>=1.12.0` in `requirements.txt` (Streamlit Cloud / REST)
- `qdrant-client==1.7.3` pinned **only** in `airflow/Dockerfile` (gRPC `UpdateMode | None` bug on Python 3.11)
- Added `rembg>=2.0.0` and `onnxruntime>=1.17.0` for virtual try-on background removal

### Secrets management

Secrets are now handled via `utile._get_secret(key, default)` — reads `st.secrets` first, falls back to `os.environ`. All modules use this instead of direct `os.getenv`. See `README_SECRETS.md`.

### Airflow integration (unchanged)

The DAG `fashion_pipeline_v2`, Docker Compose stack (7 services), and Streamlit → Airflow REST API trigger all remain as designed in sections 4–8 above. Airflow timeouts in app.py now default to 3 s with a graceful "offline" message.
